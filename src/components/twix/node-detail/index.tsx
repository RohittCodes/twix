...
import { ArrowUpRightSquareIcon, Lightbulb } from 'lucide-react';
...
export default function NodeDetail() {
  ...
  const [suggestedPhases, setSuggestedPhases] = useState<string>("");
  ...
  const handleSuggestions = async () => {
    if (!selectedNode || !selectedNode.data || !selectedNode?.data.label) {
      console.error("Selected node or node label is undefined");
      return;
    }
    ...
    try {
      ...
      if (res && res.response) {
        ...
        setSuggestedPhases(response.text());
      } else {
  ...
  return (
    <Panel position="bottom-center">
      <Card>
        ...
        <Button size={'icon'} variant={'ghost'} onClick={() => handleSuggestions()}>
            <div className="flex items-center" onClick={() => setSuggesting(true)}>
            <Lightbulb />
            </div>
          </Button>
          </div>
      ...
      <Sheet open={suggesting} onOpenChange={(val) => setSuggesting(val)}>
        <SheetContent className="p-0 sm:max-w-xl">
          ...
          <ScrollArea className="h-[calc(100vh_-_55px)]">
            <div className="min-h-[500px] p-2 text-md">
              {
                loadingSuggestions ? (
                  <div>Loading...</div>
                ) : (
